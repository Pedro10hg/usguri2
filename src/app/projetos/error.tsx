...
export default function ProjetosError({ reset }: { error: Error; reset: () => void }) {
  return (
    <section className="flex min-h-[50vh] items-center py-20">
      <Container className="text-center">
        <AlertTriangle className="mx-auto mb-4 h-12 w-12 text-red-500" />
        <h2 className="text-2xl font-bold">Erro ao carregar rolês</h2>
        <p className="mx-auto mt-2 max-w-md text-slate-600 dark:text-slate-400">
          Não foi possível carregar os rolês do grupo. Tente novamente.
        </p>
        <button
          onClick={reset}
